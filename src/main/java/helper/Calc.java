...
*/
  ...
  */
  ...
  public static com.sun.star.awt.Point getCellPos(XSpreadsheet sheet, String cellName)
  {
    ...
    com.sun.star.awt.Point pos = 
           (com.sun.star.awt.Point) Props.getProperty(xCell, "Position");
    if (pos == null) {
  ...
  public static int columnStringToNumber(String colStr)
  {
    ...
    for (int i=0; i < len; i++) {
      int val = (int)(colStr.charAt(i) - 'A' + 1);
      col = col*26 + val;
    }
    return col-1;
  ...
  { System.out.println("Cell: Sheet" + (addr.Sheet+1) + "." +
                             getCellStr(addr));
  } 
  ...
  {
    System.out.println("Range: Sheet" + (crAddr.Sheet+1) + "." +
           getCellStr(crAddr.StartColumn, crAddr.StartRow) + ":" +
  ...
  public static boolean isEqualAddresses(CellRangeAddress addr1, CellRangeAddress addr2)
  {
     if ((addr1 == null) || (addr2 == null))
       return false;
     return ((addr1.Sheet == addr2.Sheet) &&
             (addr1.StartColumn == addr2.StartColumn) &&
             (addr1.StartRow == addr2.StartRow) &&
             (addr1.EndColumn == addr2.EndColumn) &&
  ...
  // Returns the cell range as a string
  {
    return getCellStr(crAddr.StartColumn, crAddr.StartRow) + ":" +
           getCellStr(crAddr.EndColumn, crAddr.EndRow);
  }  // end of getRangeStr()
  ...
  // Returns the cell range as a string
  {
    return getCellStr(startCol, startRow) + ":" +
           getCellStr(endCol, endRow);
  }  // end of getRangeStr()
  ...
  /* Columns are numbered starting at 0 where 0 corresponds to A 
     ...
     i.e. convert a base 10 number to base 26. 
  */
  { 
    String colStr = "";
     while (nColumn >= 0) { 
  ...
  public static void changeStyle(XSpreadsheet sheet, int x1, int y1, 
                                                     int x2, int y2, String styleName)
     ...
     top-left cell of the range.  */
  {
    ...
    XCellRange headerRange = getCellRange(sheet, addr.StartColumn, addr.StartRow,
  ...
  public static void insertScenario(XSpreadsheet sheet, String rangeStr,
                                 Object[][] vals, String scenName, String comment)
     ...
     applies the value array */
  {
    ...
    XCellRangeAddressable addr =
                 Lo.qi(XCellRangeAddressable.class, cellRange);
  ...
  public static void applyScenario(XSpreadsheet sheet, String name)
  {
    try {
      ...
      XScenario scenario = Lo.qi(XScenario.class, 
                                          scenarios.getByName(name));
      scenario.apply();
    }
    catch(Exception e)
  ...
  {
    try {
      XFunctionAccess fa = Lo.createInstanceMCF(XFunctionAccess.class,
  ...
  public static void printFunArguments(PropertyValue[] propVals)
  {
    FunctionArgument[] fargs = 
             (FunctionArgument[]) Props.getValue("Arguments", propVals);
    if (fargs == null) {
  ...
  */
  ...
  public static SolverConstraint makeConstraint(XSpreadsheet sheet, String cellName, 
                                                                   String op, double d)
