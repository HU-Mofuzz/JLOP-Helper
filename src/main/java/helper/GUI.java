...
public class GUI
{
  ...
  /* Add a user-defined icon and command to the start of the
     specified toolbar.
     Based on http://www.oracle.com/technetwork/java/javase/
                          downloads/menus-toolbars-139540.html
  ...
  */
  { String cmd = Lo.makeUnoCmd(itemName);
    ...
    try {
      ...
      XImageManager imageMan = 
                Lo.qi(XImageManager.class, confMan.getImageManager());
      String[] cmds = {cmd};
  ...
  public static void showMessageBox(String title, String message)
  {
    ...
    desc.WindowAttributes = WindowAttribute.BORDER | WindowAttribute.MOVEABLE |
  ...
  /* if you want to use handles, use the JNAUtils library which allows code
     to use HWND structures */ 
  {
    XWindow win = getWindow(doc);
    XSystemDependentWindowPeer winPeer = Lo.qi(XSystemDependentWindowPeer.class, win);
    int handle = (Integer) winPeer.getWindowHandle( new byte[8], 
  ...
  // zoom to a specific value using a dispatch
  {
    ...
    /*
      Could not get types 1, 2, 3 to work; tried value = -32768, 0, 100
      Type = 1 'for Fit Optimal
      Type = 2 'for Fit Width_and_Height
      Type = 3 'for Fit Width
      Type = 0 'for Fit user defined %, set value = 72 for 72%
    */
  ...
  public static XUIConfigurationManager getUIConfigManagerDoc(XComponent doc)
  {
    ...
    XModuleUIConfigurationManagerSupplier xSupplier =
          Lo.createInstanceMCF(XModuleUIConfigurationManagerSupplier.class, 
  ...
  public static void printUICmds(XUIConfigurationManager configMan, 
                                 String uiElemName)
