...
public class Images
{
  ...
  /* assume 96 dpi,
     so 96 pixels == 1 inch == 25.4 mm == 2540 office 1/100 mm units
     s0 1 pixel == 2540/96 == 26.46 office 1/100 mm units
  */
  ...
  // load the graphic as a bitmap, and return it as a string
  {
    try {
      XNameContainer bitmapContainer =  Lo.createInstanceMSF(
              XNameContainer.class, "com.sun.star.drawing.BitmapTable");
...
/*
  public static Size getImageSize(String fnm)
  // return the slide dimensions of the image in 1/100 mm units
  {
    BufferedImage im = loadImage(fnm);
    if (im == null)
      return null;
    else {   // convert pixel dimensions into slide units
      int w = (int)Math.round(im.getWidth()*PIXEL_SCALE);
      int h = (int)Math.round(im.getHeight()*PIXEL_SCALE);
      return new Size(w,h);
    }
  }  // end of getImageSize()
*/
  ...
  // imFormat can be be "gif", "png", "jpeg", "wmf", "bmp", "svg", etc)
  {
    ...
    PropertyValue[] pngProps =  Props.makeProps("URL", FileIO.fnmToURL(fnm), 
